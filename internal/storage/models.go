...
type QueuedResult struct {
	...
	// ContentHash is the hash of the file content when analyzed.
	ContentHash string

	// ResultJSON is the serialized AnalysisResult.
	ResultJSON []byte

	// Status is the queue item status: pending, inflight, completed, failed.
	Status QueueStatus

	// RetryCount is the number of persistence attempts.
	RetryCount int
	...
	// StartedAt is when processing started (set when status becomes inflight).
	StartedAt *time.Time

	// CompletedAt is when processing completed (set when status becomes completed/failed).
	CompletedAt *time.Time
}

// QueueStatus represents the status of a queued persistence item.
type QueueStatus string

const (
	// QueueStatusPending indicates the item is waiting to be processed.
	QueueStatusPending QueueStatus = "pending"

	// QueueStatusInflight indicates the item is currently being processed.
	QueueStatusInflight QueueStatus = "inflight"

	// QueueStatusCompleted indicates the item was successfully processed.
	QueueStatusCompleted QueueStatus = "completed"

	// QueueStatusFailed indicates the item failed after max retries.
	QueueStatusFailed QueueStatus = "failed"
)

// QueueStats contains statistics about the persistence queue.
type QueueStats struct {
	...
	// Completed is the count of successfully processed items.
	Completed int64

	// Failed is the count of items that failed after max retries.
	Failed int64

	// OldestPending is the enqueue time of the oldest pending item.
	OldestPending *time.Time
}

// Total returns the total count of all items in the queue.
func (s *QueueStats) Total() int64 {
	return s.Pending + s.Inflight + s.Completed + s.Failed
}
