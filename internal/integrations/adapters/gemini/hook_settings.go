...
// readHookSettings reads the Gemini CLI settings file for hooks
// Returns the parsed settings, the full settings map (for preserving other fields), and any error
func readHookSettings(path string) (*GeminiSettings, map[string]any, error) {
	...
	if hooksData, ok := fullSettings["hooks"]; ok {
		...
		var hooksMap map[string][]GeminiHookEvent
	...
	if settings.Hooks == nil {
		settings.Hooks = make(map[string][]GeminiHookEvent)
	}

	return &settings, fullSettings, nil
}

// writeHookSettings writes the Gemini CLI settings file
// Performs atomic write with backup
func writeHookSettings(path string, settings *GeminiSettings, fullSettings map[string]any) error {
